...
| --- | --- | --- |
| Images to PDF | One PDF per input, one page matching the oriented image aspect ratio; preserve resolution and alpha where supported. A combined ordered PDF is a separate focused action requiring explicit ordering | Page size/count, orientation and ICC/color checks; rendered comparison for opaque/transparent inputs; reliable Unicode names; validated copy publication |
