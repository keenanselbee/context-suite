"""Inspect authored ExcelDateSnapshots copies; no engine or formula is executed."""
...
def main():
    ...
    if report["Mode"] != "ExcelDateSnapshots" or len(report["Results"]) != 6 or {(item["Source"], item["ProfileStyle"]) for item in report["Results"]} != cases:
    ...
    for item in report["Results"]:
        ...
        checked.append({"Source": name, "ProfileStyle": profile, "SourceSha256": item["SourceSha256"],
                        ...
                        "StoredDates": item["StoredDates"], "Cells": observed})
    result = {"Mode": report["Mode"], "ExpectedCachesMatch": all(cell["Matches"] for item in checked for cell in item["Cells"]),
              "Results": checked, "Scope": "Separate command-line workbook copies only. No same-instance PDF correspondence, repaired rendering, production guard or complete formatting interpretation."}
