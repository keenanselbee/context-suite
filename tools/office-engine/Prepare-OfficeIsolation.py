...
def prepare(prepared, destination):
    ...
    if len(relative.parts) != 2 or not re.fullmatch("[0-9a-f]{32}", relative.parts[0]) or relative.parts[1] != "office":
        raise ValueError("Use the new isolation case's office directory")
    if destination.exists():
        raise ValueError("Never reuse an isolation runtime copy")
    ...
    receipt = {"prepared": str(prepared), "destination": str(destination), "archiveSha256": pin["archiveSha256"],
    ...
    (destination.parent / "office-copy.json").write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(f"Verified isolated Office copy: {len(paths)} files", flush=True)


if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit("Usage: Prepare-OfficeIsolation.py <prepared Office directory> <new case/office>")
