...
$prepared = (Resolve-Path -LiteralPath $(if ($isCurated) { $CandidateDirectory } else { $PreparedDirectory })).Path
$candidateRoot = [IO.Path]::GetFullPath((Join-Path $repository '.codex-temp')) + '\'
$allowedRoot = if ($isCurated) { $candidateRoot } else { $audioRoot }
...
if ($ArtworkFixture) {
    ...
    if ((-not $artwork.StartsWith($audioRoot, [StringComparison]::OrdinalIgnoreCase) -and
         -not ($isCurated -and $artwork.StartsWith($prepared + '\', [StringComparison]::OrdinalIgnoreCase))) -or
...
Write-Output "Isolated audio worker payload and results: $scratch"
